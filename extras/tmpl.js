/**
 * @license RequireJS sproutcore tmpl plugin 0.26.0
 *
 * You'll need to make sure you do something like, or similar to:
 * `npm install sc-handlebars -g` 
 *
 * This is licensed under the WTFPL
 *
 * Inspired by the require.js text! plugin by James Burke
 *
 * Require stuff is all
        ...
        text = {
            ...
            finishLoad: function (name, strip, content, onLoad, config) {
                ...
                require( ['sproutcore'], function ( SC ) {
                  ...
                  var scName = name.replace( /(^\/)|(.*\/)|(\.\S*$)/g, '' );
                  // console.log( 'compile and register', scName, content );
            ...
            write: function (pluginName, moduleName, write, config) {
                var SCHB = require.nodeRequire('sc-handlebars');
                if (moduleName in buildMap) {
                    var content = text.jsEscape(buildMap[moduleName]);
                    var scName = moduleName.replace( /(^\/)|(.*\/)|(\.\S*$)/g, '' );
                    write("define( '" + pluginName + "!" + moduleName + "', ['sproutcore', 'Handlebars'], function ( SC, Handlebars ) {\n" +
                             "SC.TEMPLATES['" + scName + "'] = Handlebars.template(" + SCHB.precompile( content.replace( /\s+/g, " " ) ).replace(/\\n/g, "\n") + ");\n" +
                             "return SC.TEMPLATES['" + scName + "'];" +
                           "});\n");
                }
            },
